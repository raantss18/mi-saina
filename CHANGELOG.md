...
- **Installeur `.run` : crash au lancement sur install GNOME fraîche** ([issue #3](https://github.com/raantss18/mi-saina/issues/3)) — le binaire desktop pré-compilé charge dynamiquement `libappindicator3.so.1` (barre système), absente par défaut sur Fedora et d'autres distros → panique `Failed to load ayatana-appindicator3 or appindicator3 dynamic library`. L'installeur `.run` (`packaging/setup.sh`) installe désormais le **runtime graphique** (webkit + appindicator + librsvg) adapté à la distribution détectée (best-effort). L'install depuis les sources (`install.sh`) n'était pas touchée (ses paquets `-devel` tirent déjà le runtime).
